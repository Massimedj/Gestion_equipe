       const SETS = ['set1', 'set2', 'set3', 'set4', 'set5'];
		...
		function toggleAttendanceList() {
  		  ...
  		  // Affiche ou cache la liste des joueurs
  		  list.classList.toggle('hidden');
    
 		   // Fait pivoter l'icône de la flèche
		...
		function openInfoFaultsModal() {
  		  document.getElementById('infoFaultsModal').classList.remove('hidden');
		}
...
function deletePlayer(playerId) {
    ...
    if (confirm('Voulez-vous vraiment supprimer ce joueur ? Il sera retiré de tous les matchs.')) {
        ...
        function renderPlayerList() {
            ...
            const sortedPlayers = [...currentTeam.players].sort((a, b) => (parseInt(a.jerseyNumber) || Infinity) - (parseInt(b.jerseyNumber) || Infinity));
...
function renderCourt() {
    ...
    if (!matchId || !currentTeam) {
        ...
        document.getElementById('libero-selector-container').innerHTML = ''; // Vider le sélecteur de libéro
        return;
    }
    ...
    const onCourtPlayerIds = Object.keys(matchSetPositions)
                                .filter(pos => pos.startsWith('pos-'))
                                .map(pos => matchSetPositions[pos]);

    const liberoId = matchSetPositions.libero;
    const presentPlayers = currentTeam.players.filter(p => match.present.includes(p.id));

    // Rendu des 6 postes du terrain
    for (let i = 1; i <= 6; i++) {
        const posDiv = document.getElementById(`pos-${i}`);
        ...
        presentPlayers.forEach(player => {
            // Un joueur est dispo pour le terrain s'il n'est pas le libéro
            if (player.id === liberoId) return;

            if (player.id === currentPlayerIdForThisPos || !onCourtPlayerIds.includes(player.id)) {
    ...
    liberoSelect.className = 'player-select w-full p-2';
    liberoSelect.dataset.position = 'libero';
    liberoSelect.innerHTML = '<option value="">-- Choisir un libéro --</option>';

    presentPlayers.forEach(player => {
		// Un joueur est dispo pour être libéro s'il n'est pas sur le terrain ET si son poste est Libéro
		const isLibero = player.mainPosition === 'Libéro' || player.secondaryPosition === 'Libéro';
		if (isLibero && (player.id === liberoId || !onCourtPlayerIds.includes(player.id))) {
    ...
    liberoSelect.addEventListener('change', updatePosition);
...
function updatePosition(event) {
    ...
    // Gérer les conflits : un joueur ne peut pas être sur le terrain ET libéro
    if (playerId) {
        if (position === 'libero') {
            // Si on nomme un joueur libéro, on le retire du terrain
            for (const pos in setPositions) {
            ...
            // Si on met un joueur sur le terrain, on le retire du poste de libéro
    ...
    recalculatePlayedStatus(matchId);
    saveData();
    renderAttendanceForSelectedMatch(); // Ceci va appeler renderCourt() pour tout rafraîchir
}
...
function addFaultButtonListeners(button, matchId, playerId, faultType) {
    ...
    const startPress = (e) => {
        ...
        pressTimer = setTimeout(() => {
            ...
            // On empêche le comportement par défaut (sélection de texte, etc.)
            e.preventDefault(); 
    ...
    const movePress = (e) => {
        ...
        // Tolérance au mouvement augmentée à 10px pour plus de souplesse
    ...
    // Écouteurs tactiles sans l'option "passive: true"
    button.addEventListener('touchstart', startPress);
    button.addEventListener('touchend', endPress);
    button.addEventListener('touchmove', movePress);
}
...
function renderLiveTrackingView() {
    ...
    const onCourtPlayerIds = Object.keys(setLineup)
        .filter(pos => pos.startsWith('pos-'))
        .map(pos => setLineup[pos]);
    const liberoId = setLineup.libero;

    if (onCourtPlayerIds.length === 0 && !liberoId) {
        ...
        if (player) {
            ...
            if (match.faultTrackingMode === 'detailed') {
                ...
                ['service', 'attack', 'reception', 'net'].forEach(type => {
                    ...
                    if (button) { // Vérification pour éviter une erreur avec le libéro
                ...
                addFaultButtonListeners(button, match.id, player.id, 'net');
            }
            return card;
        }
        return null;
    ...
    // Affiche les 6 joueurs du terrain
    onCourtPlayerIds.forEach(createAndAppendCard);

    // Affiche le libéro en bas et visuellement séparé
    if (liberoId) {
        const liberoCard = createAndAppendCard(liberoId);
        if (liberoCard) {
            // CETTE LIGNE AJOUTE LE CADRE JAUNE, quel que soit le mode (simple ou détaillé)
            liberoCard.classList.add('border-t-4', 'border-yellow-500', 'mt-2');
        }
    }
...
function createPlayerCardLiveDetailed(match, player) {
    ...
    // --- NOUVELLE LOGIQUE ---
    // 1. On vérifie si le joueur a le poste de Libéro
    const isLibero = player.mainPosition === 'Libéro' || player.secondaryPosition === 'Libéro';

    let buttonsHtml = '';
    // 2. On affiche les boutons en fonction de son poste
    if (isLibero) {
        // Pour un libéro, on n'affiche que les fautes de Réception et Autres
        buttonsHtml = `
            <div class="flex flex-col gap-2 mt-4">
                <button id="fault-btn-${player.id}-reception" class="fault-grid-btn w-full py-3">
                    Réception: <span class="fault-count" id="fault-count-${player.id}-reception">${faults.reception}</span>
                </button>
                <button id="fault-btn-${player.id}-net" class="fault-grid-btn w-full py-3">
                    Autres: <span class="fault-count" id="fault-count-${player.id}-net">${faults.net}</span>
                </button>
            </div>
        `;
    } else {
        // Pour les autres joueurs, on affiche les quatre types de fautes
        buttonsHtml = `
    ...
    return card;
        ...
        function updateFault(matchId, playerId, faultType, change) {
			...
			// Si on décrémente, on fait vibrer le téléphone
  			  if (change < 0) {
   			     if (navigator.vibrate) { // On vérifie si le navigateur supporte la vibration
    			        navigator.vibrate(100); // Vibration de 100 millisecondes
    			    }
    			}
...
function openSubstitutionModal(playerIdToReplace) {
    ...
    // --- NOUVELLE LOGIQUE ---
    // On détermine si le joueur à remplacer est le libéro
    const isReplacingLibero = (playerIdToReplace === liberoId);

    // On récupère tous les joueurs sur le banc
    let availableSubstitutes = currentTeam.players.filter(p => 
        match.present.includes(p.id) && 
        !onCourtPlayerIds.includes(p.id) &&
        p.id !== liberoId
    );

    // On filtre la liste en fonction du rôle du joueur sortant
    if (isReplacingLibero) {
        document.getElementById('substitutionModalTitle').textContent = `Remplacer le Libéro`;
        // Si on remplace le libéro, on ne montre que les autres libéros disponibles
        availableSubstitutes = currentTeam.players.filter(p => {
            const isLiberoPlayer = p.mainPosition === 'Libéro' || p.secondaryPosition === 'Libéro';
            return isLiberoPlayer && !onCourtPlayerIds.includes(p.id) && p.id !== playerIdToReplace;
        });
    }
    // --- FIN DE LA NOUVELLE LOGIQUE ---
